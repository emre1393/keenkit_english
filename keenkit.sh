#!/bin/sh
# translated via vscode-copilot and disabled some lines that sends logs to "spatiumstas" url. 
...
packages_delete() {
  ...
  if [ -z "$removed_packages" ] && [ -z "$failed_packages" ]; then
    print_message "Used packages are not installed" "$CYAN"
  fi
...
perform_dd() {
  ...
  if echo "$output" | grep -iq "error\|can't"; then
    print_message "Error while overwriting partition" "$RED"
    umountFS
    exit_function
  fi
}
...
change_country() {
  if get_country; then
    print_message "Router region is RU, it is recommended to change to EA" "$CYAN"
    read -p "Change country? (y/n) " user_input
...
backup_config() {
  if has_an_external_storage; then
    ...
    y | Y)
      echo ""
      select_drive "Select a drive for backup:"
...
show_progress() {
  ...
  while [ "$downloaded" -lt "$total_size" ]; do
    if [ -f "$file_path" ]; then
      ...
      printf "\rDownloading $file_name... (%d%%)" "$progress"
    fi
    sleep 1
  done
  printf "\n"
...
ota_update() {
  ...
  if [ -z "$DIRS" ]; then
    ...
    print_message "Error $http_text while retrieving data, please try again later" "$RED"
    exit_function
    fi
    ...
    if echo "$DIR_NUM" | grep -qE '^[0-9]+$' && [ "$DIR_NUM" -ge 1 ] && [ "$DIR_NUM" -le "$dir_count" ]; then
      ...
      print_message "Invalid selection. Choose from 1 to $dir_count." "$RED"
    fi
    done
    ...
    if [ -z "$BIN_FILES" ]; then
    printf "${RED}No files found in directory $DIR.${NC}\n"
    exit_function
    else
    printf "\nFirmwares for $DIR:\n"
    i=1
    ...
    while true; do
      ...
      print_message "Invalid selection. Choose from 1 to $file_count." "$RED"
      fi
    done
...
firmware_manual_update() {
  ...
  if [ "$ram_size" -lt $MIN_RAM_SIZE ]; then
    ...
    output=$(mount)
    select_drive "Select the drive containing the update file:"
    ...
    echo ""
    read -p "$(printf "Selected ${GREEN}$FirmwareName${NC} for update, is this correct? (y/n) ")" item_rc1
...
backup_block() {
  ...
  read -p "Enter partition number(s), separated by spaces: " choice
  echo ""
  choice=$(echo "$choice" | tr -d '\n\r')
  ...
  if [ "$error_occurred" -eq 0 ] && [ $valid_parts -eq 1 ]; then
    ...
    print_message "Error during saving. Check the output above." "$RED"
  fi
  exit_function
}
...
rewrite_block() {
  check_host
  output=$(mount)
  select_drive "Select the drive containing the file:"
  files=$(find_files "$selected_drive" "60k")
  count=$(echo "$files" | wc -l)
  if [ -z "$files" ]; then
    print_message "File for replacement not found in the selected storage" "$RED"
    exit_function
  fi
  echo "Files found:"
  echo "$files" | sed "s|$selected_drive/||" | awk '{print NR".", $0}'
  ...
  for part in $choice; do
    if [ "$part" = "0" ]; then
      print_message "Bootloader will not be overwritten!" "$RED"
      continue
    fi
...
service() {
  ...
  if [ $? -ne 0 ] || ! head -n1 "$SCRIPT_PATH" | grep -q "^#\|^import\|^def\|^class"; then
    print_message "Error receiving file, please try again later" "$RED"
    exit_function
  fi
  ...
  if [ -n "$mtdSlot" ]; then
    ...
    if [ $? -eq 0 ]; then
      print_message "Backup of current U-Config saved to $folder_path" "$GREEN"
    else
